...
const TechDebate = () => {
  ...
  const [techDebateData, setTechDebateData] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const querySnapshot = await getDocs(collection(db, "tech-debate"));
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        if (editMode) {
          const participantRef = doc(db, "tech-debate", currentEditId);
          await updateDoc(participantRef, formData);
        } else {
          const collectionRef = collection(db, "tech-debate");
  ...
  const handleEdit = (id) => {
    const participant = techDebateData.find((item) => item.id === id);
  ...
  const handleSort = (column) => {
    ...
    const sortedData = [...techDebateData].sort((a, b) => {
  ...
  {!formVisible && !editMode && (
      ...
      {/* Board displaying Tech Debate participants */}
      <div className="data-board">
        <h3>Participants List</h3>
        {loading ? (
          <p>Loading...</p>
        ) : techDebateData.length === 0 ? (
          ...
          <table className="data-table">
            ...
            <tbody>
              {techDebateData.map((participant) => (
