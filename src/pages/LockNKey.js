...
const LockNKey = () => {
  ...
  const [LockNKeyData, setLockNKeyData] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const querySnapshot = await getDocs(collection(db, "lock-n-key"));
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        if (editMode) {
          const participantRef = doc(db, "lock-n-key", currentEditId);
          await updateDoc(participantRef, formData);
        } else {
          const collectionRef = collection(db, "lock-n-key");
  ...
  const handleEdit = (id) => {
    const participant = LockNKeyData.find((item) => item.id === id);
  ...
  const handleSort = (column) => {
    ...
    const sortedData = [...LockNKeyData].sort((a, b) => {
  ...
  {!formVisible && !editMode && (
      ...
      {/* Board displaying Lock N Key participants */}
      <div className="data-board">
        <h3>Participants List</h3>
        {loading ? (
          <p>Loading...</p>
        ) : LockNKeyData.length === 0 ? (
